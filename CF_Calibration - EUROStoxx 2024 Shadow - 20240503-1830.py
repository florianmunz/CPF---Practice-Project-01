...
Shadow Approach with Data from EUROStoxx 50 - Jan 2024 Data
"""
...
# ----- Adj Data for Euro Stoxx 50 Jan 2024 (Practice Project) ------------ #

# Start = pd.Timestamp('2020-05-15')
# End =  pd.Timestamp('2020-06-19')


S0 = 4663.5                         # EURO STOXX 50 level
...
# Maturity Date by-passed via Excel Adjustment ---------------------------- #

# Maturity_Date = pd.Timestamp('2024-02-16 16:00:00')
...
def H93_calibration_full():
    ...
    np.save('opt_sv_EUROStoxx', np.array(opt))
    return opt
...
#
kappa_v, theta_v, sigma_v, rho, v0 = np.load('opt_sv_EUROStoxx.npy')
...
def BCC_calibration_short():
    ...
    np.save('opt_jump_EUROStoxx', np.array(opt2))
    return opt2
...
kappa_v, theta_v, sigma_v, rho, v0 = np.load('opt_sv_EUROStoxx.npy')
lamb, mu, delta = np.load('opt_jump_EuroStoxx.npy')
...
def BCC_calibration_full():
    ...
    np.save('opt_full_EUROStoxx', np.array(opt))
    return opt
...
filename = 'cal_results_full_EUROStoxx_final.h5'
...
# Code for Opening the h5 file and read out into variables --------------- #

# h5 = pd.HDFStore('cal_results_full_EUROStoxx_final.h5', 'r')
# z = h5['options']
...
options = calculate_implied_volatilities('cal_results_full_EUROStoxx_final.h5')
options


# Test Calcs on Implied Vola --------------------------------------------- #
...
calc_imp_vols(options)



options.set_index('Strike')[['Imp_Vol', 'Imp_Vol_Mod', 'Imp_Vol_Mar']].plot(
...
options['Imp_Vol'] = options_call['Imp_Vol'] / 100

options.set_index('Strike')[['Imp_Vol', 'market_iv', 'model_iv']].plot(
                figsize=(10, 6), style=['b^', 'ro']);
